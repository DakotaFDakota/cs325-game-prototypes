export default class EatPretzelScene extends Phaser.Scene{
    ...
    create(){
        ...
        if(playerinj > 0 && playerinj < 2){
            this.titletext = this.add.text(50,150,"" + this.player1 + " takes damage from", {font: "20px Courier"});
            this.player1health = this.player1health - 1;
        }
        if(playerinj < 3 && playerinj > 1){
            this.titletext = this.add.text(50,150,"" + this.player2 + " takes damage from", {font: "20px Courier"});
            this.player2health = this.player2health - 1;
        }
        if(playerinj < 4 && playerinj > 2){
            this.titletext = this.add.text(50,150,"" + this.player3 + " takes damage from", {font: "20px Courier"});
            this.player3health = this.player3health - 1;
        }
        if(playerinj < 5 && playerinj > 3){
            this.titletext = this.add.text(50,150,"" + this.player4 + " takes damage from", {font: "20px Courier"});
        ...
        this.option1.on('pointerdown', () => this.scene.start("EatPretzelScene", {cb: this.crowbar,   num: this.numplayers, playname1: this.player1, playname2: this.player2, playname3: this.player3, playname4: this.player4, player1h: this.player1health,player2h: this.player2health, player3h: this.player3health, player4h: this.player4health}));
